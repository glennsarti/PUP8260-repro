...
Puppet::Type.newtype(:fixed_namevar_parent) do
  @doc = <<-EOT
  fixed_namevar_parent
  EOT
  ...
  def eval_generate
    ...
    should_values = catalog
                      ...
                      .select {|dep| dep.type == :fixed_namevar_child }
    ...
    is_values.each do |is_value|
      unless should_values.include?(is_value.downcase)
        ...
        resources << Puppet::Type.type(:fixed_namevar_child).new(:title => unique_key, :path => self[:path], :value_name => is_value, :ensure => :absent, :catalog => catalog)
      end
    end
